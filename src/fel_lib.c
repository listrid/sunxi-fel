...
static void usb_bulk_send(libusb_device_handle *usb, int ep, const char *data,
			  size_t length, bool progress)
{
	...
	while (length > 0) {
		...
		rc = libusb_bulk_transfer(usb, ep, (unsigned char*)data, chunk,
					  &sent, USB_TIMEOUT);
		if (rc != 0)
...
static void usb_bulk_recv(libusb_device_handle *usb, int ep, char *data,
			  int length)
{
	int rc, recv;
	while (length > 0) {
		rc = libusb_bulk_transfer(usb, ep, (unsigned char*)data, length,
					  &recv, USB_TIMEOUT);
		if (rc != 0)
			   ...
			   uint32_t *dst, size_t count)
{
	...
	uint32_t buffer[LCODE_MAX_WORDS];
