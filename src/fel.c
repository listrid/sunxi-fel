...
static uint32_t crc32(const uint8_t* data, int size)
{
    uint32_t r = ~0;
...
unsigned int file_size(const char *filename)
{
    ...
    long size = ftell(fp); // Get the current position (which is the size)
...
void aw_fel_hexdump(feldev_handle *dev, uint32_t offset, size_t size)
{
	...
	while(size > 0)
	{
		...
		if(s > 1600)
			s = 16000;
		aw_fel_read(dev, offset, buf, s);
		hexdump(buf, offset, s);
		size -= s;
		offset += s;
	}
}
...
void aw_fel_fill(feldev_handle *dev, uint32_t offset, size_t size, unsigned char value)
{
    ...
    while(size > 0)
    {
        ...
        if(s > 1600)
            s = 16000;
		memset(buf, value, s);
		aw_write_buffer(dev, buf, offset, s, false);
        size -= s;
        offset += s;
    }
}
